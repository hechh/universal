...
func NewSngTexasGame(data *pb.TexasRoomData) *SngTexasGame {
	...
	// 启动定时器
	head := &pb.Head{SendType: pb.SendType_POINT, ActorName: "SngTexasGame", FuncName: "OnTick"}
...
// 计算获胜者筹码
func (d *SngTexasGame) Reward(wins map[uint64]struct{}, chips map[uint64]int64, srvs map[uint64]int64) (total int64) {
...
func (d *SngTexasGame) SitDownReq(head *pb.Head, req *pb.TexasSitDownReq, rsp *pb.TexasSitDownRsp) error {
	...
	// 广播消息
	newHead := &pb.Head{
		Src: framework.NewSrcRouter(pb.RouterType_RouterTypeRoomId, d.RoomId, "SngTexasGame", "SitDownReq"),
...
func (d *SngTexasGame) StandUpReq(head *pb.Head, req *pb.TexasStandUpReq, rsp *pb.TexasStandUpRsp) error {
	...
	// 广播消息
	newHead := &pb.Head{
		Src: framework.NewSrcRouter(pb.RouterType_RouterTypeRoomId, d.RoomId, "SngTexasGame", "StandUpReq"),
		Cmd: uint32(pb.CMD_TEXAS_EVENT_NOTIFY),
		Uid: head.Uid,
	}
