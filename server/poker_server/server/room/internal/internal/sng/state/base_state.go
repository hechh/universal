...
func (d *BaseState) OnTick(nowMs int64, curState pb.GameState, extra interface{}) pb.GameState {
	...
	if duration := util.GetCurStateTTL(machineCfg, curState) + machine.GetCurStateStartTime() - nowMs; duration <= 0 {
	...
	defer func() {
		...
		framework.NotifyToClient(uids, newHead, sng.NewTexasEventNotify(pb.TexasEventType_EVENT_BET, event))
	}()
	...
	default:
		...
		if !(table.CurState == pb.GameState_SNG_TEXAS_PRE_FLOP && bigBlind == usr.GameInfo.BetChips && next.Uid == bigPlayer.Uid) &&
