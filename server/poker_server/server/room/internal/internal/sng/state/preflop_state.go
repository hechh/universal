...
func (d *PreflopState) OnEnter(nowMs int64, curState pb.GameState, extra interface{}) {
	...
	ttl := machineCfg.PreFlopDuration*1000 + machine.GetCurStateStartTime() - nowMs
	...
	room.Walk(int(small.GameInfo.Position), func(usr *pb.TexasPlayerData) bool {
		room.Deal(1, func(cursor uint32, cardVal uint32) {
			...
			// 发送广播
			framework.NotifyToClient([]uint64{usr.Uid}, newHead, sng.NewTexasEventNotify(pb.TexasEventType_EVENT_DEAL, &pb.TexasDealEventNotify{
				RoomId:        room.RoomId,
