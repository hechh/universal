...
func (d *RummySettleState) OnEnter(nowMs int64, curState pb.GameState, extra interface{}) {
	...
	game.Walk(0, func(player *pb.RummyRoomPlayer) bool {
		if player.State != pb.RummyPlayState_Rummy_PLAYSTATE_WIN {
			coin := game.Data.RoomCfg.BaseScore * player.Private.Score
			player.Coin -= uint64(coin)

			playerSettle := &pb.RummySettlePlayerInfo{
	...
	// 赢家拿下奖池
	winnerSettle := &pb.RummySettlePlayerInfo{
		PlayerId:  winner.PlayerId,
		CardGroup: winner.Private.CardGroup,
		Score:     game.Data.Common.OprPlayer.ScorePool,
		State:     winner.State,
		Coin:      winCoin,
		BonusCash: int64(winner.Coin),
	}
	...
	head := framework.NewHead(dst, pb.RouterType_RouterTypeDataType, game.GetRoomId())
	...
	err = framework.Send(head, matchReq)
	if err != nil {
	...
	err = framework.Send(head, settleReq)
	if err != nil {
