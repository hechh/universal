...
func NewRummyGame(data *pb.RummyRoomData) *RummyGame {
	...
	if data.Common == nil {
		data.Stage = pb.GameState_Rummy_STAGE_INIT
		data.Common = &pb.RummyCommon{
			CreateTime: time.Now().UnixMilli(),
			PlayerIds:  make([]uint64, 0, data.RoomCfg.MaxPlayerCount),
			Players:    make(map[uint64]*pb.RummyRoomPlayer, data.RoomCfg.MaxPlayerCount),
	...
	if err != nil {
		mlog.Debugf("register timer err: %v", err)
	}
...
func (d *RummyGame) OnTick() {
	...
	if d.isChange {
		...
		head := framework.NewHead(dst, pb.RouterType_RouterTypeRoomId, d.GetId())
		if err := framework.Send(head, d.Data); err != nil {
...
// todo 获取大厅玩家数据
func (d *RummyGame) GetOrNewPlayer(playerId uint64) (ret *pb.RummyRoomPlayer) {
...
func (d *RummyGame) UpReadyPlayer() {
	d.Change()
	if int32(d.GetCurState()) < int32(pb.GameState_Rummy_STAGE_START) {
		...
		for i := range players {
			...
			if player.State == pb.RummyPlayState_Rummy_PLAYSTATE_PLAY { // 正常游戏滚场自动添加就绪玩家
...
func (d *RummyGame) DelExitPlayer(playerID uint64, nowMs int64) {
	if playerID != 0 {
		d.delTargetPlayer(playerID)
		return
	}
...
func (d *RummyGame) delTargetPlayer(playerID uint64) {
	...
	if int32(d.machine.GetCurState()) < int32(pb.GameState_Rummy_STAGE_START) {

		player, ok := playerMap[playerID]

		if ok && player.Health != pb.RummyPlayHealth_Rummy_QUIT { // 主动退出 清桌不提示消息
			// todo 断线重连 删除房间信息
			//if err := rummy_room_player.HDel(player.PlayerID); err != nil {
			//	mlog.Errorf("玩家断线重连信息删除失败, DelExitPlayer, playerID:%v", playerID)
			//	return
			//}
			...
			d.Data.Common.EmptySeats = append(d.Data.Common.EmptySeats, player.Seat) //防止主动退出 座位回收两次
		}
...
func (d *RummyGame) SetPlayerLose(playerID uint64, isFold bool) {
...
func (d *RummyGame) NotifyToClient(uids []uint64, notifyId pb.RummyEventType, msg proto.Message) error {
	...
	newHead := &pb.Head{
		Src: framework.NewSrcRouter(pb.RouterType_RouterTypeRoomId, d.GetId()),
		Cmd: uint32(pb.CMD_RUMMY_EVENT_NOTIFY),
	}

	return framework.NotifyToClient(uids, newHead, ntf)
}
...
func (d *RummyGame) SetNextPlayer() pb.GameState {
	...
	Score := d.getGiveUpCard(d.GetCurPlayer())
...
func (d *RummyGame) getGiveUpCard(player *pb.RummyRoomPlayer) (score int64) {
	...
	if player.Private.PrevCard == 0 {
		...
		if d.Data.Common.OprPlayer.Step == pb.RummyRoundStep_PLAYSTEPDRAW {
...
// giveupCard 投降
func (d *RummyGame) giveupCard(player *pb.RummyRoomPlayer) {
	// 更新投降状态
	if player.State == pb.RummyPlayState_Rummy_PLAYSTATE_PLAY {
		d.SetPlayerLose(player.PlayerId, true)
	}
...
func (d *RummyGame) chiCard(player *pb.RummyRoomPlayer) uint32 {
	...
	if len(d.Data.Common.OprPlayer.OutCards) < 2 {
		...
		d.Data.Common.ShowCard = d.Data.Common.OprPlayer.OutCards[len(d.Data.Common.OprPlayer.OutCards)-2]
		if len(d.Data.Common.OprPlayer.OutCards)-2 > 0 {
			d.Data.Common.ShowCard2 = d.Data.Common.OprPlayer.OutCards[len(d.Data.Common.OprPlayer.OutCards)-3]
		}
	}
