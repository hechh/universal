...
func (d *SngRoomMgr) Query(head *pb.Head, roomId uint64) error {
	...
	if data == nil {
		return uerror.New(1, pb.ErrorCode_TEXAS_ROOM_NOT_FOUND, "SNG房间人数已满，无法在加入 head:%v, roomId:%d", head, roomId)
	}
	return framework.SendResponse(head, data)
}
...
func (d *SngRoomMgr) RoomListReq(head *pb.Head, req *pb.SngRoomListReq, rsp *pb.SngRoomListRsp) error {
	...
	sort.Slice(rsp.RoomList, func(i, j int) bool {
		return rsp.RoomList[i].PlayerCount < rsp.RoomList[j].PlayerCount
	})
	return nil
}
...
func (d *SngRoomMgr) build(cfgs ...*pb.TexasConfig) error {
	...
	for _, cfg := range cfgs {
		...
		rsp := &pb.GenRoomIdRsp{}
		if err := framework.Request(&pb.Head{Dst: dst}, req, rsp); err != nil {
			return err
		}
		if rsp.Head != nil {
