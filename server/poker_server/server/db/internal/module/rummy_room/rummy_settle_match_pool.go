...
func init() {
	// todo 联合索引初始化
	dao.RegisterMysqlTable(domain.MYSQL_DB_PLAYER_DATA, &pb.RummySettleMatch{})
}
...
func (d *RummySettleMatchPool) InsertPool(head *pb.Head, data []*pb.RummySettleMatch) error {
	mlog.Debugf("InsertPool data:%v", data)
	session := dao.GetMysql(domain.MYSQL_DB_PLAYER_DATA).NewSession()
	defer session.Close()
	dao.GetMysql(domain.MYSQL_DB_PLAYER_DATA).GetEngine().ShowSQL(true)
