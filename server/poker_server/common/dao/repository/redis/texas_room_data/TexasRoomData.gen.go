...
func HGetAll() (ret map[string]*pb.TexasRoomData, err error) {
	// 获取redis连接
	...
	// 解析数据
	ret = make(map[string]*pb.TexasRoomData)
...
func HMGet(fields ...string) (map[string]*pb.TexasRoomData, error) {
	if len(fields) <= 0 {
		return nil, nil
	}
	// 获取redis连接
...
func HMSet(data map[string]*pb.TexasRoomData) error {
	// 获取redis连接
	client := manager.GetRedis(DBNAME)
	if client == nil {
...
func HGet(RoomId uint64) (*pb.TexasRoomData, bool, error) {
	// 获取redis连接
	client := manager.GetRedis(DBNAME)
	if client == nil {
...
func HSet(RoomId uint64, data *pb.TexasRoomData) error {
	// 获取redis连接
	client := manager.GetRedis(DBNAME)
	if client == nil {
		return uerror.New(1, pb.ErrorCode_CLIENT_NOT_FOUND, "redis数据库不存在: %s", DBNAME)
