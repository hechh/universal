...
func (d *TexasGame) String() string {
	buf, _ := json.Marshal(d.TexasRoomData)
	return fmt.Sprintf("updateTime:%d, isChagne:%v, Room:%s", d.updateTime, d.isChange, string(buf))
}

func (d *TexasGame) Init() {
	head := &pb.Head{SendType: pb.SendType_POINT, ActorName: "TexasGame", FuncName: "OnTick"}
	d.RegisterTimer(head, 50*time.Millisecond, -1)
}
...
func (d *TexasGame) Finish() {
	mlog.Infof("----------------------游戏结束，房间正被回收(%d)-------------------------", d.RoomId)
	d.FinishFlag++
}

func (d *TexasGame) HasFinished() bool {
...
func (d *TexasGame) Save() {
	...
	if err := room_util.DeepCopy(d.TexasRoomData, newData); err != nil {
		mlog.Errorf("序列化数据失败%v", d.TexasRoomData)
		return
	}
	newData.Table.GameData = nil
	newData.Table.CurState = pb.GameState_TEXAS_INIT
	for _, usr := range newData.Table.Players {
		usr.GameInfo = nil
	}

	// 更新房间数据
	matchType, gameType, coinType := room_util.TexasRoomIdTo(d.RoomId)
	newHead := &pb.Head{
		Src: framework.NewSrcRouter(d.GetId(), d.GetActorName()),
		Dst: framework.NewMatchRouter(room_util.ToMatchGameId(matchType, gameType, coinType), "MatchTexasRoom", "Update"),
	}
	if err := cluster.Send(newHead, &pb.UpdateTexasRoomDataNotify{Data: newData}); err != nil {
...
func (d *TexasGame) Destroy() error {
	// 结算完成的房间，一定要删除房间
	matchType, gameType, coinType := room_util.TexasRoomIdTo(d.RoomId)
	newHead := &pb.Head{
		Src: framework.NewSrcRouter(d.GetId(), d.GetActorName()),
		Dst: framework.NewMatchRouter(room_util.ToMatchGameId(matchType, gameType, coinType), "MatchTexasRoom", "Delete"),
	}
	if err := cluster.Send(newHead, &pb.DeleteTexasRoomDataNotify{RoomId: d.RoomId}); err != nil {
		return err
	}
	...
	for _, usr := range d.Table.Players {
		...
		cluster.SendToClient(&pb.Head{
			...
			Cmd: uint32(pb.CMD_TEXAS_QUIT_ROOM_NOTIFY),
		}, &pb.TexasQuitRoomNotify{})

		// 结算筹码
		event := &pb.TexasFinishNotify{
			RoomId: d.RoomId,
			PropId: uint32(coinType),
			Incr:   usr.Chips,
		}
		err := cluster.Send(&pb.Head{
			Uid: usr.Uid,
			Src: framework.NewSrcRouter(usr.Uid, d.GetActorName()),
			Dst: framework.NewGameRouter(usr.Uid, "PlayerMgr", "TexasFinishNotify"),
		}, event)
		mlog.Infof("德州结算筹码:%d(%d), uid<%d>, event<%v>, error<%v>", d.RoomId, d.GameId, usr.Uid, event, err)
	}
	d.Table.Players = nil
	d.Table.ChairInfo = nil

	// 上报通知
	if cfg := texas_config.MGetID(d.GameId); cfg != nil {
		newHead := &pb.Head{
			Src: framework.NewSrcRouter(d.GetId(), d.GetActorName()),
			Dst: framework.NewDbRouter(uint64(pb.DataType_DataTypeReport), "ReportDataMgr", "TexasRoomReport"),
		}
		cluster.Send(newHead, &pb.TexasRoomReport{
			RoomId:            d.RoomId,
			GameType:          gameType,
			RoomType:          cfg.RoomType,
			CoinType:          coinType,
			Blind:             fmt.Sprintf("%d/%d", cfg.SmallBlind, cfg.BigBlind),
			CreateTime:        d.CreateTime,
			FinishTime:        time.Now().Unix(),
			TotalRound:        d.Table.Round,
			TotalBuyinChips:   d.TotalBuyinChips,
			TotalJoinCount:    uint32(len(d.Table.Players)),
			TotalServiceChips: d.TotalServiceChips,
			TotalRuningWater:  d.TotalRuningWater,
		})
	}
	return nil
}
...
func (d *TexasGame) GetGamePlayers() (users []*pb.TexasPlayerData) {
	mlog.Debugf("ChairInfo: %v", d.Table.ChairInfo)
	for _, uid := range d.Table.ChairInfo {
...
func (d *TexasGame) getWinners(pot *pb.TexasPotData) (users []*pb.TexasPlayerData) {
...
func (d *TexasGame) Reward(wins map[uint64]int64, chips map[uint64]int64, srvs map[uint64]int64) (total int64) {
	...
	for _, pot := range d.Table.GameData.PotPool.PotList {
		drop := true
		usr := d.Table.Players[pot.UidList[0]]
		if len(pot.UidList) == 1 && usr.GameInfo.BetChips == pot.Chips {
			drop = false
		}
		users := d.getWinners(pot)
		lusers := int64(len(users))
		add := pot.Chips/lusers - pot.Chips/int64(len(pot.UidList))
		if drop && int(lusers) == len(pot.UidList) {
			add = pot.Chips
		}
		vsrv := int64(0)
		if drop && (!isNoFlopNoDrop || (isNoFlopNoDrop && len(d.Table.GameData.PublicCardList) > 0)) {
...
func (d *TexasGame) StatisticsReq(head *pb.Head, req *pb.TexasStatisticsReq, rsp *pb.TexasStatisticsRsp) {
...
func (d *TexasGame) JoinRoomReq(head *pb.Head, req *pb.TexasJoinRoomReq, rsp *pb.TexasJoinRoomRsp) error {
	if d.FinishFlag > 0 {
		return uerror.New(1, pb.ErrorCode_TEXAS_ROOM_HAS_DESTROY, "房间已经解散")
	}
	...
	if !ok {
		if texasCfg.MaxRoomPlayerCount <= uint32(len(d.Table.Players)) {
			return uerror.New(1, pb.ErrorCode_TEXAS_ROOM_MAX_PLAYER_LIMIT, "房间最大人数限制")
		}
	...
	machi := d.GetMachine()
	if machi != nil {
		rsp.Duration = util.GetCurStateTTL(machineCfg, d.Table.CurState) - (time.Now().Unix()*1000 - machi.GetCurStateStartTime())
	} else {
...
func (d *TexasGame) QuitRoomReq(head *pb.Head, req *pb.TexasQuitRoomReq, rsp *pb.TexasQuitRoomRsp) error {
	...
	if usr.ChairId > 0 {
		...
		newRsp := &pb.TexasStandUpRsp{}
		if err := d.StandUpReq(&pb.Head{Uid: head.Uid}, &pb.TexasStandUpReq{RoomId: req.RoomId, ChairId: usr.ChairId}, newRsp); err != nil {
			return err
		}
		if newRsp.Head != nil {
			return uerror.ToError(newRsp.Head)
		}
		rsp.ChairId = newRsp.ChairId
		rsp.RoomId = newRsp.RoomId
	}
...
func (d *TexasGame) SitDownReq(head *pb.Head, req *pb.TexasSitDownReq, rsp *pb.TexasSitDownRsp) error {
	...
	// 广播消息
	newHead := &pb.Head{
		Src: framework.NewSrcRouter(d.RoomId, "TexasGame", "SitDownReq"),
		Cmd: uint32(pb.CMD_TEXAS_EVENT_NOTIFY),
		Uid: head.Uid,
	}
	mlog.Info(head, "SitDown: %v", usr)
...
func (d *TexasGame) StandUpReq(head *pb.Head, req *pb.TexasStandUpReq, rsp *pb.TexasStandUpRsp) error {
	...
	// 广播消息
	newHead := &pb.Head{
		Src: framework.NewSrcRouter(d.RoomId, "TexasGame", "StandUpReq"),
		Cmd: uint32(pb.CMD_TEXAS_EVENT_NOTIFY),
		Uid: head.Uid,
	}
	...
	if req.OperateType != int32(pb.OperateType_CHECK) && req.OperateType != int32(pb.OperateType_FOLD) {
		...
		default:
			if usr.Chips < req.Chip || usr.Chips == req.Chip {
				...
				req.Chip = usr.Chips
			} else if d.Table.GameData.MaxBetChips > 0 {
