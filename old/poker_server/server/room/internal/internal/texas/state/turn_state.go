...
func (d *TurnState) OnEnter(nowMs int64, curState pb.GameState, extra interface{}) {
	...
	defer func() {
		...
		mlog.Debugf("roomId:%d,round:%d,Turn OnEnter: %s", room.RoomId, room.Table.Round, string(buf))
	}()
	...
	room.Deal(1, func(cursor uint32, cardVal uint32) {
		if texasCfg.IsTest {
			if testCfg := texas_test_config.MGetRound(texas_test_config.MGetRoundKey(room.Table.Round)); testCfg != nil {
				strCard := util.Index[string](testCfg.Publics, 3, "")
