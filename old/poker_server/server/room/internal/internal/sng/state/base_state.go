...
func (d *BaseState) OnTick(nowMs int64, curState pb.GameState, extra interface{}) pb.GameState {
	...
	defer func() {
		...
		cluster.SendToClient(newHead, sng.NewTexasEventNotify(pb.TexasEventType_EVENT_BET, event), uids...)
	}()
	...
	case pb.OperateType_FOLD:
		room.Table.GameData.RankUidList = append(room.Table.GameData.RankUidList, usr.Uid)
		// 无任何活跃玩家
		if next == nil {
