...
import (
	...
	"poker_server/server/match/internal"
)
...
func main() {
	...
	// 加载游戏配置
	yamlcfg, node, err := yaml.LoadConfig(cfg, pb.NodeType_NodeTypeMatch, int32(nodeId))
	if err != nil {
		panic(fmt.Sprintf("游戏配置加载失败: %v", err))
	}
	nodeCfg := yamlcfg.Match[node.Id]
	...
	if err := redis.Init(yamlcfg.Redis); err != nil {
