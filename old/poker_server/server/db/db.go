...
import (
	...
	"poker_server/server/db/internal"
)
...
func main() {
	...
	// 加载游戏配置
	yamlcfg, node, err := yaml.LoadConfig(cfg, pb.NodeType_NodeTypeDb, int32(nodeId))
	if err != nil {
		panic(fmt.Sprintf("游戏配置加载失败: %v", err))
	}
	nodeCfg := yamlcfg.Db[node.Id]
	...
	util.Must(mysql.Init(yamlcfg.Mysql))

	// 初始化框架
	mlog.Infof("启动框架服务")
	util.Must(cluster.Init(node, nodeCfg, yamlcfg))
