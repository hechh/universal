...
var (
	gates    []uint64
	platform uint64
	gatecfg  map[uint32]*config.ServerConfig
...
func Init(cfg map[uint32]config.Config, plat uint64, srvs string) {
	platform = plat
	gatecfg = cfg
...
func Send(wg *sync.WaitGroup, name, strJson string, cb domain.ApiCallback) error {
	id := protocol.GetCrc(name)
	pac := protocol.GetProto(id)
	if pac == nil {
...
func Login(uid uint64, cb domain.ApiCallback) {
	// 创建玩家
	if pl, err := NewPlayer(uid, RandomGate(uid), closes); err != nil {
		cb(&domain.ApiResult{UID: uid, Error: err})
	} else {
		// 添加管理
		AddPlayer(pl)
		// 开始登录
