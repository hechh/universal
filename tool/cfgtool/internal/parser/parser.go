		...
		for _, val := range items {
			...
			*/
			...
			case "@config":
				...
				manager.AddTable(file, strs[1][:pos], domain.TypeOfConfig, strs[1][pos+1:], data, util.Suffix(strs, 2))
			}
		}
	}
	return nil
		...
		case "map":
			cfg.AddIndex(&base.Index{
				...
				Type: &base.Type{
					...
					TypeOf:  util.Or[int](len(keys) > 1, int(domain.TypeOfStruct), int(domain.TypeOfBase)),
