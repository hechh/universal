...
func (d *TurnState) OnEnter(nowMs int64, curState pb.TexasGameState, room domain.IRoom) {
	...
	defer func() {
		...
		mlog.Debugf("machine %d Turn OnEnter: %s", roomData.RoomId, string(buf))
	}()
	...
	room.Deal(1, func(cursor uint32, card uint32) {
		record.Detail.DealList = append(record.Detail.DealList, &pb.TexasGamePokerDealRecord{
			DealType: pb.TexasDealType_TDT_TURN,
			Card:     card,
			Cursor:   cursor,
		})
