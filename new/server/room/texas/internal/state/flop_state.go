...
type TexasFlopState struct{ BaseState }

func (d *TexasFlopState) OnEnter(nowMs int64, curState pb.TexasGameState, room domain.IRoom) {
	...
	defer func() {
		...
		mlog.Debugf("machine %d Flop OnEnter: %s", roomData.RoomId, string(buf))
	}()

	// 发送3张公共牌
	room.Deal(3, func(cursor uint32, card uint32) {
		record.Detail.DealList = append(record.Detail.DealList, &pb.TexasGamePokerDealRecord{
			DealType: pb.TexasDealType_TDT_FLOP,
			Card:     card,
			Cursor:   cursor,
		})
