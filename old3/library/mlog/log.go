...
// ---------Debugh---------
func Debugh(head *pb.Head, format string, args ...interface{}) {
	if filter.IsFilter(head) {
		logger.Debug(1, filter.Filter(head, format), args...)
	}
}

func Traceh(head *pb.Head, format string, args ...interface{}) {
	if filter.IsFilter(head) {
		logger.Trace(1, filter.Filter(head, format), args...)
	}
}

func Warnh(head *pb.Head, format string, args ...interface{}) {
	if filter.IsFilter(head) {
		logger.Warn(1, filter.Filter(head, format), args...)
	}
}

func Infoh(head *pb.Head, format string, args ...interface{}) {
	if filter.IsFilter(head) {
		logger.Info(1, filter.Filter(head, format), args...)
	}
}

func Errorh(head *pb.Head, format string, args ...interface{}) {
	if filter.IsFilter(head) {
		logger.Error(1, filter.Filter(head, format), args...)
	}
}

func Fatalh(head *pb.Head, format string, args ...interface{}) {
	if filter.IsFilter(head) {
		logger.Fatal(1, filter.Filter(head, format), args...)
	}
}
