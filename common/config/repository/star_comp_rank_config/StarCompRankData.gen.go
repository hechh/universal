...
var obj = atomic.Pointer[StarCompRankConfigData]{}
...
func LGet() (rets []*pb.StarCompRankConfig) {
	if data := obj.Load(); data != nil {
		rets = make([]*pb.StarCompRankConfig, len(data._List))
...
func MGetPrizeTypeLevel(PrizeType int32, Level int32) *pb.StarCompRankConfig {
	...
	if val, ok := data._PrizeTypeLevel[pb.Index2[int32, int32]{PrizeType, Level}]; ok {
		return val
	}
	return nil
}
...
func GGetPrizeType(PrizeType int32) (rets []*pb.StarCompRankConfig) {
	...
	if vals, ok := data._PrizeType[PrizeType]; ok {
