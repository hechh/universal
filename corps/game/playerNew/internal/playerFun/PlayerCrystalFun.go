...
func NewPlayerCrystalFun(uid uint64, mgr *manager.FunMgr) domain.IPlayerFun {
	item := &PlayerCrystalFun{PlayerFun: NewPlayerFun(uid, pb.PlayerDataType_Crystal, mgr)}
	mgr.RegisterReward(cfgEnum.ESystemType_Crystal, item.AddCrystal)
	return item
}
...
// 新系统
func (this *PlayerCrystalFun) NewPlayer() error {
...
// 加载数据(非system类型数据)
func (this *PlayerCrystalFun) Load(pData []byte) error {
	if len(pData) <= 0 {
		this.NewPlayer()
		return nil
	}
	...
	if err := proto.Unmarshal(pData, pbData); err != nil {
		return uerror.NewUErrorf(1, cfgEnum.ErrorCode_DATABASE, "proto解析PBPlayerCrystal出错: %v", err)
	}
...
// 客户端数据
func (this *PlayerCrystalFun) SaveDataToClient(pbData *pb.PBPlayerData) error {
...
// 设置玩家数据, web管理后台
func (this *PlayerCrystalFun) SetUserTypeInfo(buf []byte) error {
	data := &pb.PBPlayerCrystal{}
	if err := proto.Unmarshal(buf, data); err != nil {
		return uerror.NewUErrorf(1, cfgEnum.ErrorCode_ProtoUnmarshal, "proto解析PBPlayerCrystal数据出错：%v", err)
	}
...
func (this *PlayerCrystalFun) AddCrystal(head *pb.RpcHead, do pb.EmDoingType, extra interface{}, items ...*pb.PBAddItemData) (rets []*pb.PBAddItemData, err error) {
	for _, pbItem := range items {
		crystalID, itemCount := pbItem.Id, pbItem.Count
		star := uint32(1)
		...
		if cfg == nil {
			err = uerror.NewUErrorf(1, cfgData.GetCrystalErrorCode(crystalID), "head: %v, item: %v", head, pbItem)
			return
		}
		// 判断晶核是否解锁
		...
		if !ok {
			// 解锁晶核
			this.unlockCrystal(head, cfg, star)
			itemCount--
			crystal = this.mapCrystal[crystalID]
			rets = append(rets, &pb.PBAddItemData{Kind: pbItem.Kind, Id: pbItem.Id, Count: 1, DoingType: do, Params: pbItem.Params})
		}
		// 转换成其他道具
		if itemCount > 0 {
			// 晶核转成晶核碎片
			...
			rets = append(rets, &pb.PBAddItemData{Kind: uint32(cfgEnum.ESystemType_Item), Id: cfg.PieceID, Count: count, DoingType: do})
		}
		//成就触发
...
// 添加机器人
func (this *PlayerCrystalFun) AddRobot(head *pb.RpcHead, do pb.EmDoingType, extra interface{}, items ...*pb.PBAddItemData) (rets []*pb.PBAddItemData, err error) {
	for _, pbItem := range items {
		robotID := pbItem.Id
		...
		if cfg == nil {
			err = uerror.NewUErrorf(1, cfgData.GetCrystalRobotErrorCode(robotID), "head: %v, item: %v", head, pbItem)
			return
		}
		//不解锁，只给碎片
