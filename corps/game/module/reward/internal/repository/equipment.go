...
import (
	...
	"corps/pb"
	rbase "corps/server/game/module/reward/internal/base"
)

type Equipment struct{}

func (d *Equipment) Probability(isAdd bool, probs map[common.ItemIndex]int64, items ...*pb.PBAddItemData) (rets []*pb.PBAddItemData) {
	// 分类
	tmps := map[common.ItemIndex]*pb.PBAddItemData{}
	for _, item := range items {
		index := common.ItemIndex{item.Kind, item.Id, item.Equipment.Quality, item.Equipment.Star}
		if _, ok := tmps[index]; !ok {
			tmps[index] = &pb.PBAddItemData{
				Kind:      item.Kind,
				Id:        item.Equipment.Id,
				DoingType: item.DoingType,
				Params:    append([]uint32{}, item.Params...),
			}
		}
		tmps[index].Count++
	}
	...
	for index, item := range tmps {
		if isAdd {
			item.Count = rbase.AddProb(probs[index], item.Count)
		} else {
			item.Count = rbase.SubProb(probs[index], item.Count)
		}
