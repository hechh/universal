...
func HGetAll() (ret map[string]*pb.RoomInfo, err error) {
	// 获取redis连接
	...
	// 解析数据
	ret = make(map[string]*pb.RoomInfo)
...
func HMGet(fields ...string) (map[string]*pb.RoomInfo, error) {
	if len(fields) <= 0 {
		return nil, nil
	}
	// 获取redis连接
...
func HMSet(data map[string]*pb.RoomInfo) error {
	// 获取redis连接
	client := manager.GetRedis(DBNAME)
	if client == nil {
...
func HGet(uid uint64) (*pb.RoomInfo, bool, error) {
	// 获取redis连接
	client := manager.GetRedis(DBNAME)
	if client == nil {
...
func HSet(uid uint64, data *pb.RoomInfo) error {
	// 获取redis连接
	client := manager.GetRedis(DBNAME)
	if client == nil {
		return uerror.New(1, pb.ErrorCode_CLIENT_NOT_FOUND, "redis数据库不存在: %s", DBNAME)
