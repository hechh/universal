...
func init() {
	...
	if err := mysql.Init(yamlcfg.Mysql); err != nil {
		panic(fmt.Sprintf("mysql初始化失败: %v", err))
	}
	token.Init(yamlcfg.Common.SecretKey)

	// 配置初始化
	if err := config.Init(yamlcfg.Etcd, yamlcfg.Data); err != nil {
...
func TestRummyConfig(t *testing.T) {
	rets := rummy_config.LGet()
	mlog.Infof("rets:%v", rets)
}
...
func TestRummySettleMatch(t *testing.T) {
	...
	head := framework.NewHead(dst, 0, pb.RouterType_RouterTypeDataType, 1)
...
func TestRummySettleSelect(t *testing.T) {
	...
	err := cluster.Request(head, req, rsp)
