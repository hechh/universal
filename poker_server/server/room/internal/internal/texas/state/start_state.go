...
func (d *StartState) OnEnter(nowMs int64, curState pb.GameState, extra interface{}) {
	...
	defer func() {
		mlog.Debugf("roomId:%d,round:%d,Start OnEnter: %s", room.RoomId, room.Table.Round, room.String())
	}()
	...
	for i, usr := range users {
		usr.TotalTimes++
		if usr.GameInfo == nil {
			usr.GameInfo = &pb.TexasPlayerGameInfo{GameState: curState, Position: uint32(i)}
		} else {
...
func (d *StartState) OnTick(nowMs int64, curState pb.GameState, extra interface{}) pb.GameState {
	...
	defer func() {
		mlog.Debugf("roomId:%d,round:%d,Start OnTick: %s", room.RoomId, room.Table.Round, room.String())
	}()
	...
	cluster.SendToClient(newHead, texas.NewTexasEventNotify(pb.TexasEventType_EVENT_GAME_START, &pb.TexasGameEventNotify{
		RoomId:        room.RoomId,
...
func (d *StartState) OnExit(nowMs int64, curState pb.GameState, extra interface{}) {
	room := extra.(*texas.TexasGame)
	defer func() {
		mlog.Debugf("roomId:%d,round:%d,Start OnExit: %s", room.RoomId, room.Table.Round, room.String())
	}()
}
