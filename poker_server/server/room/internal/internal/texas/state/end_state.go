...
func (d *EndState) OnEnter(nowMs int64, curState pb.GameState, extra interface{}) {
	...
	cluster.SendToClient(newHead, texas.NewTexasEventNotify(pb.TexasEventType_EVENT_GAME_END, event), uids...)
	...
	room.Walk(0, func(usr *pb.TexasPlayerData) bool {
		if usr.ChairId > 0 && (usr.PlayerState == pb.PlayerStatus_PlayerStatusQuitGame || texasCfg.BigBlind > usr.Chips) {
			cluster.SendToClient(newHead, texas.NewTexasEventNotify(pb.TexasEventType_EVENT_STAND_UP, &pb.TexasPlayerEventNotify{
				RoomId:     room.RoomId,
				ChairId:    usr.ChairId,
				Uid:        usr.Uid,
