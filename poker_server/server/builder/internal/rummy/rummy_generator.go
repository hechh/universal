...
func (g *BuilderRummyGenerator) Load() error {
	g.loadFlag = 1
	return actor.RegisterTimer(&g.loadFlag, func() { g.SendMsg(&pb.Head{FuncName: "LoadRequest"}) }, 5*time.Second, -1)
}
...
func (g *BuilderRummyGenerator) LoadRequest() error {
	...
	return cluster.Send(&pb.Head{
		Src: framework.NewSrcRouter(uint64(pb.GeneratorType_GeneratorTypeRummy), "BuilderRummyGenerator", "LoadData"),
...
func (g *BuilderRummyGenerator) Save() error {
	...
	head := &pb.Head{
		...
		Src: framework.NewSrcRouter(uint64(pb.GeneratorType_GeneratorTypeRummy), g.GetActorName()),
	}
