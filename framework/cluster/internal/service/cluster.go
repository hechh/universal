...
func InitCluster(node *pb.ClusterNode, natsUrl string, etcds []string) error {
	...
	if err != nil {
		return basic.NewUError(2, pb.ErrorCode_NewClient, err)
	}
	// 监听
...
func (d *Cluster) Send(pac *pb.Packet) (err error) {
	...
	if err != nil {
		return basic.NewUError(2, pb.ErrorCode_Marhsal, err)
	}
	// 转发
	head := pac.Head
	...
	return basic.NewUError(2, pb.ErrorCode_NatsPublish, err)
}
