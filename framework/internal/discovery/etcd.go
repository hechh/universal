...
func NewEtcdMonitor(topic string, endpoints []string) (cli *EtcdMonitor, err error) {
...
func (d *EtcdMonitor) Watch(cls define.INode) (err error) {
...
func (d *EtcdMonitor) handleEvent(cls define.INode, events ...*clientv3.Event) {
...
func (d *EtcdMonitor) Register(cls define.INode, ttl int64) error {
	ctx := context.Background()
	tctx, cancel := context.WithTimeout(ctx, 3*time.Second)
	defer cancel()
...
func (d *EtcdMonitor) keepAlive(cls define.INode, ttl int64, keep <-chan *clientv3.LeaseKeepAliveResponse, lease clientv3.LeaseID) {
...
func (d *EtcdMonitor) Close() error {
	close(d.exit)
	d.Wait()
