...
func (d *ActorMgr) Register(ac domain.IActor) {
...
func (d *ActorMgr) Send(h domain.IHead, args ...interface{}) error {
	if _, ok := d.funcs[h.GetActorName()]; !ok {
...
func (d *ActorMgr) SendRpc(h domain.IHead, buf []byte) error {
	if _, ok := d.funcs[h.GetActorName()]; !ok {
		return uerror.New(1, -1, "%s.%s未实现", h.GetActorName(), h.GetFuncName())
	}
	if act := d.GetActor(h.GetRouteId()); act != nil {
